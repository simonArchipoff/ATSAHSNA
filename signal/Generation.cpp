...
VF chirp_angular(double w1, double w2, double duration, int sampleRate, double phase_shift){
...
VF chirp(double f1, double f2, double duration, uint sampleRate){
...
VCF chirp_complex(double f1,double f2, double duration, unsigned int sampleRate){
    VF r = chirp_angular(2*M_PI * f1,2*M_PI*f2,duration,sampleRate,M_PI/2.);
    VF i = chirp_angular(2*M_PI * f1,2*M_PI*f2,duration,sampleRate,M_PI);
    assert(r.size() == i.size());
    VCF c(r.size(),0);
#pragma omp parallel for if(r.size() > 10000)
    for(uint j = 0; j < c.size(); j++){
        c[j] = std::complex<float>(r[j],i[j]);
    }
...
VF impulse(double freqMin, double duration, uint sampleRate){
